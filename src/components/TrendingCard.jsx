...
export default function TrendingCard({movie}) {
    ...
    return (
        ...
        <div className='relative group cursor-pointer  overflow-hidden'>
            ...
            <img className='w-full h-full rounded-xl' src={movie.thumbnail.trending.large} alt={movie.title}></img>
            <BookMark movie={movie} />

            <div className="absolute left-3 top-1/2 lg:top-[60%]">
                <ul className="text-[#bbc5da] text-sm font-medium w-full flex items-center list-disc">
                    <li className="mx-3">{movie.year}</li>
                    ...
                    <li className="mx-3">{movie.rating}</li>
                </ul>
            </div>
            <p className="absolute left-3 bottom-4 sm:bottom-[15%] text-white font-medium">{movie.title}</p>
